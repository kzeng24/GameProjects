...
public interface IModel extends IViewModel {
  ...
  /**
   * Sets the row index for a guess (sets where a guess is located in the stack). The row index at
   * the very bottom is 0 and is incremented by 1 for each row on top.
   *
  ...
  /**
   * Gets the reference of the current guess (will directly mutate the current guess even if it's
   * altered outside).
   *
   * @throws IllegalStateException if the game has not started yet
   */
  ...
  /**
   * Points to a new row on top by incrementing the row index and sets the current guess to a new
   * empty guess.
   *
   * @throws IllegalStateException if the game has not started yet
   */
